...
```
composer require gsmanager/routers
```
...
In your ```index.php``` require your newly-created ```routes.php``` and call the ```Route::start()``` method. This will trigger and do the actual routing of the requests.

It's not required, but you can set `Route::setDefaultNamespace('\App\Controllers');` to prefix all routes with the namespace to your controllers. This will simplify things a bit, as you won't have to specify the namespace for your controllers on each route.

**This is an example of a basic ```index.php``` file:**

```php
<?php
use GSManager\Router\Route;
 ...
 * @return \GSManager\Http\Input\InputHandler|array|string|null
 */
...
```php
Route::get('/', [MyClass::class, 'myMethod']);
```
...
```php
Route::get($url, $callback, $settings);
Route::post($url, $callback, $settings);
Route::put($url, $callback, $settings);
Route::patch($url, $callback, $settings);
Route::delete($url, $callback, $settings);
Route::options($url, $callback, $settings);
```
...
Route::get('/user/{name?}', function ($name = 'Simon') {
    return $name;
});
```
...
```php
Route::get('/path/{fileOrFolder}', function ($fileOrFolder) {
	return $fileOrFolder;
...
```php
Route::all('/ajax/abc/123', function($param1, $param2) {
	// param1 = abc
	// param2 = 123
...
```php
Route::get('/path/{parameter}', 'VideoController@home', ['defaultParameterRegex' => '[\w\-\æ\ø\å]+']);
```
...
```php
Route::group(['defaultParameterRegex' => '[\w\-\æ\ø\å]+'], function() {

    Route::get('/path/{parameter}', 'VideoController@home');
...
```php
Route::group(['middleware' => \App\Middleware\Auth::class], function () {
    Route::get('/', function ()    {
...
For example if your route has an absolute callback like `\App\Controller\DefaultController@home`, the namespace from the route will not be prepended.
...
```php
Route::group(['prefix' => '/lang/{lang}'], function ($language) {
...
```php
Route::group(['prefix' => '/lang/{language}'], function ($language) {
    Route::get('/users', function ($language)    {
...
```php
Route::partialGroup('/plugin/{name}', function ($plugin) {
...
Route::csrfVerifier(new \App\Middlewares\CsrfVerifier());

Route::group(['middleware' => \App\Middlewares\Site::class, 'exceptionHandler' => \App\Handlers\CustomExceptionHandler::class], function() {


    Route::get('/answers/{id}', 'ControllerAnswers@show', ['where' => ['id' => '[0-9]+']]);

	/**
     * Class hinting is supported too
     */
     
     Route::get('/answers/{id}', [ControllerAnswers::class, 'show'], ['where' => ['id' => '[0-9]+']]);

    /**
     * Restful resource (see IRestController interface for available methods)
     */

    Route::resource('/rest', ControllerResource::class);
    ...
    Route::controller('/animals', ControllerAnimals::class);

});

Route::get('/page/404', 'ControllerPage@notFound', ['as' => 'page.notfound']);
```
...
```php
return Route::router()->getCsrfVerifier()->getTokenProvider()->getToken();
```
...
use GSManager\Http\Middleware\BaseCsrfVerifier;
...
```php
$verifier = new \App\Middlewares\CsrfVerifier();
...
use GSManager\Http\Middleware\IMiddleware;
use GSManager\Http\Request;
...
If you simply want to catch a 404 (page not found) etc. you can use the `Route::error($callback)` static helper method.
...
```php
Route::get('/not-found', 'PageController@notFound');
Route::get('/forbidden', 'PageController@notFound');
...
use GSManager\Http\Request;
use GSManager\Router\Handlers\IExceptionHandler;
use GSManager\Router\Exceptions\NotFoundHttpException;
...
class CustomExceptionHandler implements IExceptionHandler
{
	public function handleError(Request $request, \Exception $error): void
	{
		...
		if($error instanceof NotFoundHttpException) {
			...
			$request->setRewriteCallback('App\Controllers\PageController@notFound');
			return;
...
```php
Route::group(['prefix' => '/', 'exceptionHandler' => \App\Handlers\FirstExceptionHandler::class, 'mergeExceptionHandlers' => false], function() {

	Route::group(['prefix' => '/admin', 'exceptionHandler' => \App\Handlers\SecondExceptionHandler::class], function() {
	
		// Both SecondExceptionHandler and FirstExceptionHandler will trigger (in that order).
	
	});
	
	Route::group(['prefix' => '/user', 'exceptionHandler' => \App\Handlers\SecondExceptionHandler::class, 'mergeExceptionHandlers' => false], function() {
...
```php
Route::get('/product-view/{id}', 'ProductsController@show', ['as' => 'product']);
...
```php
Route::controller('/images', ImagesController::class, ['as' => 'picture']);
...
```php
Route::controller('gadgets', GadgetsController::class, ['names' => ['getIphoneInfo' => 'iphone']]);
...
For more available methods please check the `GSManager\Http\Url` class.
...
/* @var $image \GSManager\Http\Input\InputFile */
...
| `EVENT_GET_URL`             | `name`<br>`parameters`<br>`getParams` | Fires whenever the `Route::getUrl` method or `url`-helper function is called and the router tries to find the route. |
...
`Route::addEventHandler()`. We recommend that you add your event-handlers within your `routes.php`.

**Example:**

```php
use GSManager\Router\Handlers\EventHandler;
...
Route::addEventHandler($eventHandler);
...
Most of the time the basic `\GSManager\Router\Handler\EventHandler` class will be more than enough for most people as you simply register an event which fires when triggered.
...
If you need multiple routes to be executed on the same url, you can enable this feature by setting `Route::enableMultiRouteRendering(true)`
...
```php
use \GSManager\Http\Middleware\IpRestrictAccess;
...
All information about the current route is stored in the `\GSManager\Router\Router` instance's `loadedRoute` property.

For easy access you can use the shortcut helper function `request()` instead of calling the class directly `\GSManager\Router\Route::router()`.
...
```php
use GSManager\Http\Request;
use GSManager\Router\IRouterBootManager;
use GSManager\Router\Router;
...
class CustomRouterRules implement IRouterBootManager 
{
     ...
     *
     * @param \GSManager\Router\Router $router
     * @param \GSManager\Http\Request $request
     */
    public function boot(\GSManager\Router\Router $router, \GSManager\Http\Request $request): void
    {
...
```php
Route::addBootManager(new CustomRouterRules());
```

### Adding routes manually

The ```Route``` class referenced in the previous example, is just a simple helper class that knows how to communicate with the ```Router``` class.
...
```php
use \GSManager\Router\Router;
use \GSManager\Router\Route\RouteUrl;
...
$route->addMiddleware(\App\Middlewares\AuthMiddleware::class);
...
```php
Route::setCustomClassLoader(new MyCustomClassLoader());
```
...
```php
use GSManager\Router\ClassLoader\IClassLoader;
use GSManager\Router\Exceptions\ClassNotFoundHttpException;
...
The framework has it's own ```Router``` class which inherits from the ```Route``` class. This allows the framework to add custom functionality like loading a custom `routes.php` file or add debugging information etc.
...
```php
Route::router()->setDebugEnabled(true);
Route::start();
```
...
```php
$messages = Route::router()->getDebugLog();
```
...
```php
Route::get('/user/{name}', 'UserController@show')->where(['name' => '[\w]+']);
```
...
Feel free to leave us feedback by [creating a new issue](https://github.com/gsmanager/routers/issues/new).
