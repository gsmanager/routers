 ...
 * ---------------------------
 *
 * This class is added so calls can be made statically like GamePLRouter::get() making the code look pretty.
...
use Closure;
use Exception;
use GSManager\Exceptions\InvalidArgumentException;
use GSManager\Http\Middleware\BaseCsrfVerifier;
use GSManager\Http\Middleware\Exceptions\TokenMismatchException;
...
use GSManager\Router\Handlers\CallbackExceptionHandler;
use GSManager\Router\Handlers\IEventHandler;
use GSManager\Router\Route\IGroupRoute;
use GSManager\Router\Route\ILoadableRoute;
use GSManager\Router\Route\IPartialGroupRoute;
use GSManager\Router\Route\IRoute;
use GSManager\Router\Route\RouteController;
use GSManager\Router\Route\RoutersGroup;
use GSManager\Router\Route\RoutePartialGroup;
use GSManager\Router\Route\RouteResource;
...
class Route
{
        ...
        if (is_file($composerFile) === true) {
            ...
            if (isset($composerInfo['packages']) === true && is_array($composerInfo['packages']) === true) {
                foreach ($composerInfo['packages'] as $package) {
                    if (isset($package['name']) === true && strtolower($package['name']) === 'GSManager\/simple-router') {
     ...
     */
    public static function get(string $url, array|string|Closure $callback, array $settings = null): IRoute
    {
     ...
     * @return IRoute
     */
    public static function match(array $requestMethods, string $url, array|string|Closure $callback, array $settings = null): IRoute
    {
